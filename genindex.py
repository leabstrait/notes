...
def generate_index(root_dir, skip_list=None):
    index_content = "# Index\n\n"
    ...
    def generate_index_recursive(dir_path, depth=0):
        ...
        for item in os.listdir(dir_path):
            ...
            if os.path.isdir(item_path):
                ...
                index_content += f"{'  ' * depth}- <details><summary>{item}</summary>\n"
                ...
                index_content += (
                    f"{'  ' * depth}- [{file_name}]({relative_path[:-3]})\n"
                )
...
# Example usage: Generate index for the "content" directory, skipping "private" directory and "secret.md" file
generate_index(".", skip_list=[".git", "CNAME", "genindex.py","notes.sh"])
